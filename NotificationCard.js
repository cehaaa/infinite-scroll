export class NotificationCard {
		...
		return `
			<figure class="flex items-start gap-2 rounded-md p-2 bg-white skeleton-card">
				...
				<div class="w-full space-y-2">
					<div class="animate-pulse h-5 bg-zinc-300 w-1/2 md:w-2/6"></div>
					<div class="animation-pulse h-5 bg-zinc-300 w-full md:w-1/2"></div>
					<div class="animation-pulse h-5 bg-zinc-300 w-full md:w-1/2"></div>
				</div>
			</figure>
		`;
	...
	render({ title, body }) {
      ...
      <figure class="bg-white flex items-start gap-2 rounded-md p-2 hover:bg-emerald-500/10 duration-200 cursor-pointer notification-card">
				<div class="size-9 rounded-full bg-white shadow flex-none hidden md:flex items-center justify-center">
